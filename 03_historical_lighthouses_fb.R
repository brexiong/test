#############################################################
### 3. Danger Zones and Restricted Areas, ak_cs_001       ###
#############################################################
...
for(i in 1:length(code_dict$study_area)){
  ...
  tryCatch({
    ...
    hex_path <- file.path(study_area_dir, code_dict$study_area[i], "2_floating_bag/d_suitability_data/constraints/1_constraints.gpkg")
    
    # layer 
    layer = paste(code_dict$code[i], "cs_fb_suitability_unconstrained", sep = "_")
    ...
    clean_data <- function(raw_data){
      data_layer <- raw_data %>%
        ...
        # create field called "layer" and fill with "historical lighthouse" for summary
        dplyr::mutate(layer = "historical lighthouse")
      return(data_layer)
    }
